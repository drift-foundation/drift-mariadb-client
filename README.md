...
`just stress` and `just perf` compile test code against the deployed signed packages, not local source trees. As of `mariadb-wire-proto` 0.6.2 / `mariadb-rpc` 0.8.2 the source tree is aligned to the 0.34.1 toolchain surface (strict `std.json.parse()` replaces the removed `parse_strict()`; the perf harness reads `driftc --version --json`), on top of the 0.33.91 borrow rules (a parameter declared `&T`/`&mut T` supplies the borrow; source-written argument borrows are rejected), so building this repo requires **driftc 0.34.1+ / ABI 22** (0.34.0 was withdrawn before certification and must not be used). Historical trust-format floor, kept for context: the trust **model** is trust-v1 (role-tagged author/certifier claims + a v1 trust store), but the signed **claim bodies are schema v2 and provenance is schema v4** as of driftc 0.33.57. Older toolchains in `[0.32.x, 0.33.56]` parse only v1 claim bodies and will reject these artifacts. (Both are higher floors than the `>= 0.33.17` shared-test-runner requirement the cert gates check.) This requires:
