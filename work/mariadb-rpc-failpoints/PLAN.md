...
- [ ] S8. Add automated proxy-process gate coverage so `just test` exercises the actual `mariadb-failpoint-proxy` binary, not only pure framing/control logic — still NOT done, do not let this slip once downstream teams start relying on the proxy during cert. Minimum v1: build local app, start proxy against `mdb114-a`, run the passthrough live client through it, assert JSONL lifecycle events including `commit_observed`, and tear down reliably. S7 is done, so the second case described in `PROXY-GATE-HARNESS.md` (armed COMMIT-loss proxy-process gate, mirroring `live_proxy_pool_commit_ambiguous_test.drift` but as an automated subprocess smoke) is now unblocked too — see `work/mariadb-rpc-failpoints/PROXY-GATE-HARNESS.md`, whose status line says "REQUIRED before treating `mariadb-failpoint-proxy` as certification-ready downstream test tooling."
