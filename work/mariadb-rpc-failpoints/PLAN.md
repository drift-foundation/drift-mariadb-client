...
`server_response_bytes_dropped` is `-1` by design, not a placeholder — but the reason differs by action (see §14's resolved drop-and-hold entry). For the default `drop_server_response_after_forward`, the proxy never reads the swallowed server response at all (reading it would itself risk forwarding part of it before closing). For `drop_and_hold_after_forward`, the proxy DOES read and discard server bytes for the duration of the hold (it has to, to drain the socket and detect the eventual close), but still never counts or forwards them. Either way an actual dropped-byte count is unmeasurable — `-1` is the documented "not measured" sentinel (see §11 S5 implementation notes).
...
- [~] S4. Implement the MariaDB wire proxy as a `kind:app` Drift artifact in this repo, mirroring uflowsd's layout: source at `failpoint-proxy/src/`, namespace `mariadb.failpoint.proxy`, app `mariadb-failpoint-proxy`, entry `mariadb.failpoint.proxy::service_main`, declared in `drift/manifest.json`. Built/certified here and consumed by the app harness as an external process (like uflowsd), not under `tools/` (which is Python helpers). Transparent forwarding with framed packet-level COM_QUERY inspection; framing inlined (trivial 4-byte MariaDB header), so no `mariadb-wire-proto` dep. Control plane raw TCP / JSON Lines, no `drift-web`.
  ...
  - Concurrent-client support is DONE (detached spawn-per-client on abi19; `TOOLCHAIN-spawn-drop-question.md` resolved). The failpoint registry, nth-from-arm COMMIT matching, JSON Lines control plane, and fault action that used to be tracked here as slices 2-3 TODO are DONE — see S5 below; remaining work is S10 (S7/S8/S9 are now also done — see below).
- [x] S5. Newline-delimited JSON control API implemented (`failpoint-proxy/src/control.drift`, module `mariadb.failpoint.proxy.control`), including S6's observability ops (delivered together — one JSON dispatch table, not worth splitting): `arm` (label + optional domain + `match.command`/`match.sql`/`match.nth` + fixed `action`/`times=1`, validated against the only supported shape — `COM_QUERY`/`COMMIT`/`drop_server_response_after_forward`), `status`, `list`, `assert_all_fired`, `clear` (full registry reset), `health`. Registry is `conc.Arc<control.Registry>` (`Mutex<RegistrySlot>` of `Array<FailpointEntry>`, linear-scan — matches this repo's existing `Array`/`Mutex` house style, no `HashMap` precedent). `match.nth` counts matching COMMITs from arm-time, per-entry (`seen_since_arm`), exactly as specified. `handle_line(reg, line) -> String` is the pure JSON-in/JSON-out core (no sockets, no logging) — unit-tested directly in `failpoint-proxy/tests/unit/control_test.drift` (arm/status/nth=1/nth=2/assert_all_fired/clear/list/health/bad-json/unknown-op/unknown-id/bad-args), the same pattern as `classify_commit_wire_tag` and the packet framer. The control TCP listener (`run_control_loop` + per-connection line reader, byte-accumulate-until-`\n`, `too-long` closes the connection since a boundary-less buffer can't safely resync) runs as a second detached accept loop in `service_main`, independent of the data listener.
  ...
  Two documented v1 approximations (diagnostics only, not gated by any DoD item): `bytes_forwarded_to_server` is hardcoded `11` (exact size of a bare `COMMIT` packet, which is what `wire.commit()` always emits — not a generic measurement); `server_response_bytes_dropped` is `-1` ("not measured" sentinel — true for v1's single action, `drop_server_response_after_forward`, which deliberately never reads the swallowed response since reading it would itself risk forwarding part of it. The later `drop_and_hold_after_forward` action DOES read and discard server bytes for the hold's duration, just still never counts/forwards them — see §14's resolved drop-and-hold entry).
...
- [x] S7. E2e test added: `packages/mariadb-rpc/tests/e2e/live_proxy_pool_commit_ambiguous_test.drift`. MANUAL (same reason as `live_proxy_passthrough_smoke_test.drift`: needs an already-running proxy in front of `mdb114-a`; not in `tools/emit_test_plan.py`'s curated `LIVE_TESTS`, so it does not run in `just test` — that automation gap is S8, tracked separately, see below). Uses a REAL `pool.ConnectionPool` (not a bare `RpcConnection`) with `min_idle=0, max_conns=1, keepalive_interval_ms=0` — the shape a real consumer uses. Flow: `clear` the proxy registry -> `arm` a one-shot exact-COMMIT failpoint over raw TCP control (the arm ack is read synchronously before the pool ever opens a connection, satisfying the arm-before-COMMIT happens-before rule in §7) -> `pool.acquire()` (first real connect, since `min_idle=0` seeds nothing) -> `conn.commit()` -> assert `RpcCommitErrorKind::AmbiguousWrite` (matches exhaustively on all three kinds, not a wildcard, so a misclassification fails loudly rather than silently passing) -> drop the lease (pool's existing `is_alive()`-gated release path discards the poisoned conn — no test-side discard call needed) -> `assert_all_fired` over control, asserted `ok:true` -> `pool.acquire()` again -> plain `commit()` asserted `Ok` (fresh physical connection, one-shot already spent). The control-plane client is a small self-contained raw-TCP/JSON-Lines helper inlined in the test file (not a dependency on `failpoint-proxy`'s `control.drift`, which belongs to a different manifest artifact). Verified live twice in a row against `mdb114-a`: proxy logs show `commit_observed`→`failpoint_fire`→`conn_close{reason:"read-error"}` on the first connection, then a second connection with `commit_observed`(no fire)→`conn_close{reason:"eof",first:"c2s"}` (clean client-driven close), proving the one-shot fired exactly once and the pool recovered cleanly. `just compile-check` clean; needed `import mariadb.rpc.managed as managed;` even though the test never spells the `managed.LeasedConn` type name — `LeasedConn.conn()`'s visibility requires the defining module imported.
  Review fixes (hang/stale-state hardening, since this test's whole point is proving a release/discard path — a regression there must fail loudly, not hang): (1) `pool.PoolConfig.acquire_timeout` defaults to `None` ("block forever" — `pool.drift:82,194`); the no-arg `p.acquire()` this test uses inherits that default, so a regressed release/discard path would hang acquire#2 instead of failing it. Set explicitly in `_pool_cfg()`: `pc.acquire_timeout = Optional::Some(conc.Duration(millis = TIMEOUT_MS))`. (2) The control-plane client's per-read timeout didn't cap the OVERALL wait — a control handler that accepted a connection but never sent a newline could retry `WOULD_BLOCK` indefinitely. Added a `time.now_monotonic()`/`time.elapsed_ms()`-tracked overall deadline (`CONTROL_OVERALL_TIMEOUT_MS = 5000`) around both `_write_all` and the response read loop. (3) The initial `clear`'s `ok` field was never checked, and an early failure after a successful `arm` could leave the shared, long-lived proxy sitting armed/fired for whatever manual test runs next. Restructured: post-arm logic moved into `_run_after_arm(p: &pool.ConnectionPool) -> Int` so every failure path funnels through one `_best_effort_clear()` call in `main()`, and the initial `clear`'s `ok` field is now checked like every other control response. Re-verified live (2 runs) and via `just compile-check` after the restructure.
- [x] S8. Automated proxy-process gate coverage added: `tools/proxy_gate_smoke.py`, invoked from `just test` (see justfile) via `flocker --key mariadb-mdb114-a -j 1` (same DB resource-key naming as `emit_test_plan.py`'s `DB_GROUP`, so it doesn't race a concurrent gate's own DB access). `just test` now fails if this step fails — it isn't a side path. Python harness chosen per `PROXY-GATE-HARNESS.md`'s own guidance ("a small Python harness is acceptable if the shared `drift_test_run.py` plan format cannot directly express 'start long-lived process A, run client B, inspect A logs, stop A'"), and it reuses `tools/emit_test_plan.py`'s `emit_app`/`emit_one` functions directly (imported, not shelled out to) so the build/run mechanics stay identical to `just build-app`/`just check-one` with no duplicated logic.
  Case 1 (v1 scope, as specified): builds the proxy from local source (no deploy/sign/publish) -> starts it as a real subprocess against `mdb114-a` with stderr captured to a file -> polls control `health` for readiness (not just a raw connect — see control.drift's `_handle_health` comment on why `health` implies the data listener is bound) -> runs the EXISTING `live_proxy_passthrough_smoke_test.drift` unmodified as a real TCP client (nothing armed; asserts connect/auth/query/commit pass through) -> asserts its exit code is 0 -> tears the subprocess down (SIGTERM, grace period, SIGKILL fallback) -> THEN asserts the captured stderr contains every expected lifecycle event at least once: `proxy_start`, `client_accept`, `backend_connect`, `commit_observed`, `conn_close` (the one thing only an outside observer of the process, not the client test, can check).
  Case 2 (now that S7 exists): a FRESH proxy instance, same readiness wait, then runs the EXISTING `live_proxy_pool_commit_ambiguous_test.drift` unmodified against the gate-managed subprocess. That test already arms the one-shot failpoint, asserts `RpcCommitErrorKind::AmbiguousWrite`, asserts fired-exactly-once via `assert_all_fired`, and proves a clean-reconnect recovery commit (see S7 above) — this harness does not reimplement any of those CLIENT-side assertions in Python — it runs the test, checks its exit code, and (per the review fix below) additionally asserts the proxy's own log, avoiding a second, driftable implementation of what the client-side test already checks while still verifying what only the process's own log can show.
  All four cases (see S9 below for cases 3/4) build the proxy ONCE (`main()`) and reuse the binary. Verified manually (`python3 tools/proxy_gate_smoke.py --work-dir ...`, no `just`/flocker wrapper) — clean pass, teardown leaves zero leftover processes; also negative-tested by injecting a bogus required event name into `REQUIRED_EVENTS_CASE1` and confirming the harness fails loudly with a clear message and still tears down cleanly (not a vacuous assertion). Wired into `just test` and verified via a full `just test` run (215 unit/e2e jobs + this new step, all green — see verification note below).
  Review fixes: (1) the shared executor's subprocess (proxy build / one client-test compile+run) had no timeout, so a hang there would keep the proxy subprocess alive forever — `finally: stop_proxy(...)` is only reached once `run_client_test` RETURNS. `_run_plan` now runs the executor with `start_new_session=True` and a bounded `timeout_s` (`BUILD_TIMEOUT_S=180`, `CLIENT_RUN_TIMEOUT_S=120` — generous over the observed ~17-20s), and on `TimeoutExpired` kills the executor's WHOLE PROCESS GROUP (not just the direct child — the executor itself spawns compiler/test-binary children a child-only kill would orphan) via `os.killpg`, then returns `None`, which the existing `if rc != 0` checks already treat as failure (`None != 0`) without needing a separate code path. Negative-tested by monkeypatching a simulated 60s hang behind a 3s timeout: the harness correctly failed after ~3s (not 60s), killed the hung process group, and still tore the proxy down cleanly — verified via `pgrep` showing zero leftover processes for both the proxy and the simulated hung child. Also added `--heartbeat {{HEARTBEAT_SECS}}` to the `flocker` invocation in `justfile` (this step previously ran without the stdout-inactivity-watchdog heartbeat the main gate uses). (2) Case 2 previously only checked the S7 test's exit code, never the proxy's own log — added `REQUIRED_EVENTS_CASE2 = [proxy_start, client_accept, backend_connect, commit_observed, failpoint_fire, conn_close]`, asserted the same way as case 1, catching a logging regression in the real binary that S7's client-side assertions can't see. Re-verified: manual run green (case 2's new assertion actually checked real log content, not vacuous), full `just test` re-run green after all fixes.
  Same fixed ports as the manual S4/S7 tests (45306 data / 45307 control / mdb114-a's 34114 backend — see §14's port-collision entry for why these moved off 43306/43307) since those Drift test files hardcode them (not parameterized via argv) — this harness must match, not the other way around.
- [x] S9. App-usage docs added: `docs/failpoint-proxy-usage.md`. Covers: starting the proxy (`just build-app` + the three listener flags with their defaults), routing (only the domain under test through the proxy, everything else direct, with the H2/H1 worked examples), the raw TCP JSON Lines control examples (`health`/`clear`/`arm`/`assert_all_fired`/`status`, with real captured request/response JSON), pool/test config recommendations (`max_conns=1`, `keepalive_interval_ms=0`, finite `acquire_timeout`), the no-failpoint-code-in-the-app-binary safety property, a security note (bind loopback by default), and a pointer to S8 (this repo's own `just test` now runs the real subprocess, not just in-process logic).
  Before finalizing this doc, added the exact day-1 coverage gaps a reviewer flagged as missing (the docs would otherwise have told downstream teams to rely on behavior that was only unit-tested, not proven against the real subprocess):
  - **nth=2 (H1) live subprocess proof**: new `packages/mariadb-rpc/tests/e2e/live_proxy_nth_commit_ambiguous_test.drift` — arms `match.nth=2`, drives TWO real COMMITs through the SAME connection (matching Singular's actual start-then-complete shape), asserts commit #1 is `Ok` (passes through untouched), commit #2 is `RpcCommitErrorKind::AmbiguousWrite` (exhaustive match), `assert_all_fired` is `ok:true`, and — the part that distinguishes "it fired" from "nth targeting actually hit the right COMMIT" — `status` shows `matched_nth == 2`. Verified live: proxy log shows `commit_observed` ordinal 1 then 2, then `failpoint_fire{matched_nth:"2"}`, both on the same `conn_id`.
  - **Domain isolation live proof**: new `packages/mariadb-rpc/tests/e2e/live_proxy_domain_isolation_test.drift` — arms `nth=1`, a "non-target domain" connection goes straight to the real backend (bypassing the proxy's data listener entirely) and commits `Ok`, then `status` confirms the armed failpoint is untouched (`armed:true, fired:false` — the direct traffic was never even observed), then a "target domain" connection goes through the proxy's data listener and its commit fires `AmbiguousWrite`, then `assert_all_fired` is `ok:true`. This repo has one dev DB, so it proves the domain-independent claim (traffic through the data listener can affect the registry; traffic that never touches it can't) with a direct-vs-proxied connection to the same backend, rather than simulating bookkeeper's actual two domains — that two-domain simulation is explicitly called out as app/workflows-owned in the doc, not left implicit. Verified live: proxy log shows exactly ONE `client_accept` (the proxied connection) — the direct connection is invisible to the proxy, as the whole routing recommendation assumes.
  - Both new tests are MANUAL (same reasons as the existing `live_proxy_*` tests) and wired as case 3 / case 4 in `tools/proxy_gate_smoke.py` (S8) via a table-driven `CASES` list + shared `run_case()` helper (replacing the case1/case2-specific functions), so this exact day-1 behavior — not just nth=1/passthrough — is exercised against the real certified subprocess automatically. Full `just test` re-verified green after adding both (215 unit/e2e jobs + all 4 S8 cases).
  Both new test files duplicate the same self-contained raw-TCP/JSON-Lines control-plane client helper as S7's `live_proxy_pool_commit_ambiguous_test.drift` (documented there as a deliberate choice — `control.drift` belongs to a different manifest artifact) rather than factoring a shared test-support module.
...
- Timeout-style ack loss (`drop-and-hold` until client timeout): implemented as action `drop_and_hold_after_forward` (`control.drift`/`main.drift`), requiring a caller-specified `hold_ms` (`1..300000`, no proxy-side default — the caller must arm a duration that outlasts whatever client commit-I/O timeout it's proving against). Proxy forwards the COMMIT, sets a per-connection `hold` flag alongside the existing `swallow` flag, sleeps `hold_ms`, then lets `_handle_client`'s existing unconditional post-pump close tear the pair down — the s2c pump keeps discarding (not returning/closing) while `hold` is set, so the connection genuinely stays open for the client to time out on, not the proxy. An app team's question — does a commit-read timeout classify identically to a reset (same `AmbiguousWrite`, no hang, pool reconnects), or does it diverge? — is answered empirically, live against a real subprocess: `packages/mariadb-rpc/tests/e2e/live_proxy_commit_timeout_ambiguous_test.drift` (wired into `tools/proxy_gate_smoke.py` as case 5), which arms `hold_ms=6000` against a `read_timeout_ms=2000` connection (COMMIT_READ_TIMEOUT_MS/PROXY_HOLD_MS — a 3x gap, not a near-miss). Verified live: client's own 2000ms read timeout fires the `AmbiguousWrite` classification at ~2.0s; the proxy's independent 6000ms hold self-closes the original (already-abandoned) connection ~4s later, fully decoupled — no divergence found, no hang, pool discards the poisoned lease and reconnects cleanly on a fresh connection.
  Review fix: asserting only the error kind and reconnect (as S7's reset-flavored test does) would NOT actually prove the timeout flavor — if `drop_and_hold_after_forward` had a bug making it silently behave like the immediate-close action, that weaker test would still see `AmbiguousWrite` and a clean reconnect and falsely pass. Added an elapsed-time assertion around `commit()` (`time.now_monotonic()`/`elapsed_ms`) asserting the failure lands in `[1500ms, 4000ms)` — ruling out both a near-instant reset-mode failure and the proxy's own hold firing first. Verified the assertion actually catches the regression it's meant to catch: temporarily reverted the hold-mode fix (fell through to the immediate-close path), rebuilt, and confirmed the test now fails loudly (`commit failed after 0ms` < the 1500ms floor) instead of falsely passing; restored the fix and reconfirmed (`commit failed after 2001ms`, inside the window). `just build-app`/unit tests (`control_test.drift` scenarios `scenario_hold_requires_hold_ms`/`scenario_hold_arm_and_fire`/`scenario_default_action_echoed`) and the full `tools/proxy_gate_smoke.py` (all 5 cases) re-verified green.
- Drift runtime signalfd busy-spin (reported to the toolchain team, see `TOOLCHAIN-signalfd-busyloop-question.md`): FIXED upstream, CERTIFIED as `drift-0.33.68+abi19` (build-orchestrator run `20260704-122605-drift-lang-95d5b19`; `~/opt/drift/certified/current` now serves it). Root cause was exactly what we reported: the reactor left the signalfd for SIGINT/SIGTERM/SIGUSR1 undrained whenever no `conc.await_signal()` waiter was registered, so `epoll_wait` returned immediately forever once one of those signals arrived — pegging a core, and since `SIGKILL` was the only way to stop an affected process, plain `SIGTERM`/Ctrl-C made things actively *worse*, not better. Fix drains the signalfd unconditionally and stashes an unconsumed signal in a small pending bitmask for a later `await_signal()` call. Scope note from the toolchain team, confirmed load-bearing: this does NOT add POSIX-default terminate-on-SIGTERM — a program (like this proxy) that never calls `await_signal()` still doesn't exit on SIGTERM, it just stops busy-spinning. Validated here (2026-07-04) against the staged toolchain: (1) the original 6-line `conc.sleep`-only repro — 0% CPU sustained 10s after SIGTERM, process stays alive; (2) the real `mariadb-failpoint-proxy` binary, unsanitized — same result; (3) built with `--sanitize address` (ASAN) — no busy-spin, zero ASAN error reports, still fully functional (drove a real client commit through post-SIGTERM, succeeded); (4) built plain and run under `valgrind --tool=memcheck --fair-sched=yes --error-exitcode=97 --leak-check=full --errors-for-leak-kinds=definite,possible` (this repo's canonical memcheck flags) — CPU flat for 12s post-SIGTERM, zero live-detected errors during both idle wait and a real client commit driven through afterward. Note: neither ASAN's leak detector nor valgrind's leak-check summary could be exercised end-to-end, because (per the scope note) this proxy never calls `await_signal()`, so SIGTERM never triggers a real exit for either tool to report against — only `SIGKILL` stops it, which bypasses both tools' exit-time reporting; this is expected, not a gap in the fix. `docs/failpoint-proxy-usage.md` corrected accordingly: use `SIGKILL`, not `SIGTERM`, to stop the proxy. Full `just test` also run against the staged toolchain to check for downstream regressions per the toolchain team's request — green, 215 unit/e2e jobs + all 5 proxy-process-gate (S8) cases, zero failures. Confirmation sent back to the toolchain team: `/tmp/drift-announce/2026-07-04T05-16-39Z-drift-mariadb-client-signalfd-fix-confirm.md`. Our own `mariadb-failpoint-proxy` version bumped 0.2.0 → 0.2.1 for the docs correction, resealed, and `just test`/`just stress`/`just perf` re-verified green on our certified toolchain (perf byte/packet counts unchanged from baseline).
- Graceful SIGTERM/SIGINT shutdown for `mariadb-failpoint-proxy` (0.2.1 → 0.2.2): an app team independently reproduced the pre-0.33.68 busy-spin against a *shipped* certified artifact (`mariadb-failpoint-proxy` 0.1.0), then — after we shipped 0.2.1 on certified 0.33.68 — reported that the busy-spin was gone but the proxy STILL never exited on SIGTERM (only `SIGKILL`), because it registered no `conc.await_signal()` waiter of its own; matches PushCoin's earlier "optional defense-in-depth" suggestion (`/tmp/drift-announce/2026-07-04T12-25-40Z-pushcoin-signalfd-busyspin-validation.md`), now made a required fix. Implemented close-immediately (no drain) semantics, matching bookkeeper/uflowsd's own ~0.22s exit time and this proxy's existing abrupt-close fault-action pattern elsewhere:
  - Both accept loops (data in `main.drift`, control in `control.drift`) now run on their OWN spawned vthread instead of one running inline as the root VT; each polls a shared `conc.Arc<AtomicBool>` `shutting_down` flag once per bounded accept() timeout (`ACCEPT_TIMEOUT_MS` dropped from 3600000ms to 200ms in both files — this is now the worst-case shutdown-notice latency, not an "expect nothing for an hour" tuning) and returns once it's set.
  - `service_main`'s root VT becomes the shutdown coordinator (`std_concurrent.md`'s single-waiter rule requires exactly one VT to own `conc.await_signal()`): loops on `await_signal()`, logs and ignores `ProcessSignal::User1` (SIGUSR1 is reserved for application use the proxy doesn't have, e.g. a reload trigger — not a shutdown signal), and on `Interrupt`/`Terminate` sets `shutting_down`, joins both accept-loop vthreads with a bounded `join_timeout` (`SHUTDOWN_JOIN_TIMEOUT_MS = 3000`, a safety net — a stuck accept loop must not hang process exit), then returns 0. In-flight per-connection/per-control-request handler vthreads are NOT drained or waited on.
  - New events: `shutdown_signal`, `signal_ignored`, `accept_loop_stopped`, `control_accept_loop_stopped`, `proxy_shutdown_complete`.
  - Regression coverage: `tools/proxy_gate_smoke.py` cases 6/7 start a fresh proxy, send SIGTERM/SIGINT directly (not through the existing grace+SIGKILL-fallback `stop_proxy` helper), and assert exit code 0 within `SHUTDOWN_EXIT_TIMEOUT_S = 3.0s` (comfortably above the observed ~0.21s, far below the ~7s the pre-fix bug needed) plus the full shutdown-event lifecycle in the log. Verified live: both signals exit in ~0.21s with exit code 0; a real client test (`live_proxy_passthrough_smoke_test.drift`) still passes against the rebuilt binary; full `tools/proxy_gate_smoke.py` (now 7 cases) and `control_test.drift` unit tests green.
  - Lower-priority, same report — a downstream harness found a stray leftover proxy instance squatting on the default data port but forwarding to a *different* backend, silently misdirecting traffic. Addressed with a new `backend_listener` field on `Registry`, threaded into `health`/`status`/`list` (docs updated with a "check this before arming" callout and a recommendation to prefer unique/ephemeral ports per harness over the doc's shared example ports).
  - `mariadb-failpoint-proxy` version bumped 0.2.1 → 0.2.2. Resealed; `just test`/`just stress`/`just perf` re-verified green on the certified toolchain.
  - Port-collision finding materialized for real, mid-validation: while re-running `just test` for this same fix, a DIFFERENT concurrent session on this shared box was found squatting on `127.0.0.1:43306`/`:43307` — the exact example ports `docs/failpoint-proxy-usage.md` shows — with a different `--backend-port`, causing our own S8 gate to fail with `listen_failed`. Confirmed with the user this is another team coexisting on the same machine, not a leftover to clean up. Fix: moved this repo's OWN test-gate ports off the documented example, onto `45306`/`45307` — `tools/proxy_gate_smoke.py`'s `DATA_PORT`/`CONTROL_PORT` and the 5 hardcoded `live_proxy_*_test.drift` files' `PROXY_DATA_PORT`/`PROXY_CONTROL_PORT`/`PROXY_PORT` consts (`main.drift`'s own `DATA_PORT_DEFAULT`/`CONTROL_PORT_DEFAULT` and the docs' illustrative example both intentionally stay `43306`/`43307` — this only changes what OUR OWN CI passes via `--data-port`/`--control-port`, not the app's documented defaults). Doesn't eliminate collision risk in general (any fixed pair can still collide with another fixed pair), but stops recurrence of this specific collision — anyone else literally copying the doc's example no longer contends with our own gate.
